...
decorator_switch = config.getboolean('decorator', 'value')

snapshot = 1

def dec(f):
    def inner(*args, **kwargs):
        print('Begging of block')
        fw = open('logprocess.txt', 'w')
        fmt1 = '%Y-%m-%d %H:%M:%S %Z'
        currenttime1 = datetime.datetime.now()
        tstmp1 = datetime.datetime.strftime(currenttime1, fmt1)
        fw.write("Snapshot #{0}, Snapshot Time - {1}\n".format(snapshot, tstmp1))
...
if filetype == "txt":
    print(filetype + ' in' + timeint + ' seconds(s)')
